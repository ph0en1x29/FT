...
**JobDetail no longer preloads the entire 3,306-row parts catalog on every mount.** `usePartsForList()` was paginating four times through the parts table (1,000 rows per page) on every JobDetail open to feed a single Combobox option list — and then rendering 3,306 ComboboxOption objects into the dropdown's first open. Even with a 2-min React-Query cache, post-eviction reloads moved megabytes of payload, and the first render of that many options dropped frames. A `useSearchParts(limit)` server-side hook already existed in `hooks/useQueryHooks.ts` (line 249) but was unused at this consumer. Switched JobDetailPage to `useSearchParts(50)`: the Combobox now fires a debounced (250ms internal) server-side search on each keystroke, only ~50 rows live in memory at a time, and the dropdown opens instantly. To keep the Combobox label and `is_liquid` flag stable when the user types a new query that filters out their already-picked part, the parent stores a `selectedPartSnapshot: Part | null` state and prepends it to the rendered options list. `BulkApproveRequestsModal`'s own `usePartsForList` (it needs the full list for fuzzy keyword auto-match) is now only invoked when the modal is actually open — the parent conditionally renders the modal instead of always-mounting it.

**Van stock no longer fetched twice on every JobDetail open.** `pages/JobDetail/hooks/useJobData.ts`'s initial-load `useEffect` was calling both `loadJob()` (which already fans out to the technician's van stock as part of its parallel batch alongside service record + active rental + active vans) AND a separate `loadVanStock()`. Two parallel requests for the same row on every mount. Removed the redundant call from the initial effect; the standalone `loadVanStock` helper is still exported for post-mutation refresh paths.

**JobBoard realtime no longer firehoses non-admin technicians.** The `postgres_changes` UPDATE and INSERT subscriptions in `pages/JobBoard/hooks/useJobData.ts` had no row filter — every fleet-wide edit (admin reassignments, status flips, slot-in acks across ~834 active jobs) pushed a WebSocket message to every connected client. The handler de-noised at the JS level (`findIndex(j => j.job_id === updatedJob.job_id) === -1` returns `prevJobs` unchanged), but the wire traffic and the per-event `scheduleFetchCounts` debounce still ran. Added a role-conditional filter: when `displayRole === UserRole.TECHNICIAN`, the subscriptions filter to `assigned_technician_id=eq.${currentUser.user_id}`, cutting WS message volume to events on the tech's own primary jobs only. Helper-job assignment changes are still covered by the existing separate `job_assignments` channel filtered to the user's `technician_id`, which triggers a full refetch on any change. Admins, supervisors, ADMIN_SERVICE, and ADMIN_STORE keep the unfiltered firehose because they need fleet-wide QuickStats counts and cross-tech assignment toasts. Trade-off: helper technicians (rare) lose live status-update streams on their helper jobs — those land on the next page mount or refresh. Acceptable given the WS reduction for primary-role techs.

**PWA service worker no longer caches Supabase API responses.** Removed the `runtimeCaching` block in `vite.config.ts` matching `/supabase\.co/` with `NetworkFirst` and a 24h TTL. Real-time SaaS gets near-zero benefit from a 24h API cache: NetworkFirst always tries network first and only falls back to cache on offline, and this app has no offline UX anyway. The cost was per-request SW marshaling overhead on every API call (~5-30ms depending on Chrome version) plus the "duplicate fetch" appearance in DevTools that triggered the original prefetch hypothesis. The font cache rule (`woff2|woff|ttf` → `CacheFirst`, 30d) is genuinely cacheable and stays. `sw.js` shrunk ~20 bytes; total bundle 546.34 KB brotli'd (was 546.40 KB).

**Customer contacts + sites only fetched when the job actually has them.** `pages/JobDetail/JobDetailPage.tsx`'s two `useQuery` calls (`customer-contacts`, `customer-sites`) were gated only on `!!job?.customer_id` (every job has one) so they ran on every JobDetail mount even when the job's `contact_id` and `site_id` were null — which is the common case (most jobs don't pin a specific PIC or site). Tightened the `enabled` flag to `!!job?.customer_id && !!job?.contact_id` (and same for `site_id`) — those queries now skip entirely on jobs that don't reference them. Added `staleTime: 5 min` to both since contacts and sites change rarely.

**Inventory page no longer blanket-invalidates 4 query keys on every tab switch back to "parts".** `pages/InventoryPage/InventoryPageMain.tsx` had a `useEffect` that called `loadParts()` whenever `activeTab` changed to `'parts'`, and `loadParts` invalidated `parts-catalog` / `catalog-stats` / `part-categories` / `part-codes` simultaneously. Going `?tab=vanstock` then back to `?tab=parts` re-fetched everything regardless of cache freshness. Removed the effect — the inner queries already gate on `enabled: activeTab === 'parts'` (paused when off-tab, resumed when re-entered with normal staleTime semantics). `loadParts` stays exported and wired into the post-mutation refresh handlers (import-complete / batch-receive-success / stock-adjust-success) where forced invalidation is correct.

### Notes

The "Chrome prefetch" hypothesis is busted: the production HTML emits exactly 4 `<link rel="modulepreload">` directives for the React + Supabase + icons vendor chunks (standard Vite output, sized correctly), zero `<link rel="prefetch">`, zero `<link rel="preload">`, no Speculation Rules. The duplicate-fetch appearance in Chrome DevTools was the PWA service worker intercepting and re-issuing supabase.co requests for the now-removed runtime cache. Verified by counting `rel=\"modulepreload\"` (4), `rel=\"preload\"` (0), `rel=\"prefetch\"` (0) in `dist/index.html`.

The realtime self-echo dedupe (`lastSeenUpdatedAtRef` in `useJobDetailState`) is unaffected by these changes — still de-duping local-write echoes correctly.

`StoreQueueController` (admin store-queue page) also calls `usePartsForList`. Out of scope for this pass; admins doing store reconciliation typically expect a heavy preload anyway. Will revisit if reported as slow.
...
A multi-round adversarial review (architect + database-reviewer + ft-reviewer ×2) converged on an atomic-RPC redesign that eliminates the half-state by construction.

### Fixes

**Trigger refactored into a shared evaluator.** `validate_job_completion_requirements` was a monolithic function that raised on any of 8 gates. Extracted the gate logic into `evaluate_job_completion(p_row jobs, p_user_role text) RETURNS text` — returns the blocker reason as text, or NULL if the job is ready. The trigger is now a thin wrapper that calls the helper and `RAISE EXCEPTION` on non-NULL. Single source of truth, zero drift surface — same rationale as the `HOURMETER_EXEMPT_JOB_TYPES` grep marker pattern used elsewhere. Migration `20260507_completion_evaluator_and_readiness_rpc.sql`.

**Readiness pre-check RPC.** New `check_job_completion_readiness(p_job_ids uuid[]) RETURNS TABLE(job_id, can_complete, blocker)` — calls the evaluator without raising. SECURITY INVOKER so RLS on the read tables (job_service_records / job_parts / job_requests / job_media / job_inventory_usage) governs access; auth.uid() flows through normally. The bulk-sign banner calls this on click and passes the resolved Map to the modal. The modal renders the SPECIFIC blocker per job — "Cannot sign yet — Checklist has not been filled", "Cannot sign yet — Parts must be recorded, or explicitly mark no parts used", etc. — instead of the previous generic "missing after photo" / "check hourmeter". Techs now see exactly what to fix on the JobDetail page before swiping.

**Atomic bulk-complete RPC.** New `rpc_bulk_complete_jobs(p_job_ids, p_tech_name, p_tech_signed_at, p_customer_name, p_customer_ic, p_customer_signed_at) RETURNS TABLE(job_id, ok, blocker)`. Per-job: PL/pgSQL implicit subtransaction (BEGIN/EXCEPTION/END) → `SELECT ... FOR UPDATE NOWAIT` (returns "Job is locked by another session" on contention rather than blocking) → synthesize the post-write row state → call `evaluate_job_completion` → if blocker, return ok=false WITHOUT writing → otherwise single UPDATE writes both signatures + status, then upserts `job_service_records` for the *_signature_at timestamp columns downstream finalization reads. Failed jobs roll back their subtransaction without affecting siblings. Returns one row per input job_id. The previous 3-write modal flow (tech sig → customer sig → `updateJobStatus`) is replaced by one call. Tech-sig idempotency via `COALESCE(jobs.technician_signature, ...)` so re-entries preserve the original `signed_at` audit trail; customer-sig always overwrites because re-entries can carry different name/IC. Migration `20260507_rpc_bulk_complete_jobs.sql`.

**Modal refactored to use the atomic RPC.** `pages/JobBoard/components/BulkSignOffModal.tsx` now consumes a `readiness: Map<string, { canComplete, blocker }>` prop instead of the previous `afterPhotoJobIds: Set<string>`. Job classification is driven entirely by readiness: `eligibleJobs` (ready + tech-pending) become step-1 candidates, `techAlreadySignedJobs` (ready + tech sig present, recovery cohort) trigger `skipTechStep` mode that opens directly on the customer step, `blockedJobs` (NOT ready) render with their specific blocker text inline. On submit: single call to `bulkCompleteJobs` (`services/jobCompletionService.ts`) which proxies to the atomic RPC; the modal then renders a per-job result panel — checkmarks for successes, ⚠ + blocker text for failures — and stays open until the tech reviews. Critically, `onComplete()` is no longer called unconditionally on partial failure (closes a HIGH finding from the multi-round review where partial successes would dismiss the modal and leave failed jobs unrecoverable on re-entry). SwipeToSign completion timestamps are captured via `onTechSwipe` / `onCustomerSwipe` and piped through to the RPC as `signed_at`, preserving the audit semantic that the tech actually performed the gesture at HH:MM.

**Banner switched to readiness pre-check.** `pages/JobBoard/components/SiteSignOffBanner.tsx` calls `checkJobCompletionReadiness` on the "Sign Off" click instead of `getAfterPhotoJobIds` — single round-trip resolves all gates, not just the photo. "Loading…" button state during the call. Banner gate counts jobs missing EITHER signature (carried forward from the morning fix), so partially-signed jobs keep the banner visible until both sigs are written.

**Service wrapper.** New `services/jobCompletionService.ts` exposing `checkJobCompletionReadiness(jobIds)` and `bulkCompleteJobs({ ... })` with typed inputs/outputs. The legacy `services/jobMediaService.ts` keeps `getAfterPhotoJobIds` and the idempotent `bulkSwipeSignJobs` from the morning fix — still used by other (non-bulk) sign paths.

### Recovery

`scripts/recover_stuck_bulk_sign_jobs.cjs` — found the 16 stuck-with-both-sigs In-Progress jobs, classified each via the new readiness RPC, and flipped 8 that the trigger would now accept (BEH CHOON SHYAN's JOB-260420-043 / JOB-260420-003-43 partner / others, GOH YUEH HAN ×3, MUHAMMAD FIRDAUS ×1, plus Hafiz's JOB-260422-056) to Awaiting Finalization. The remaining 8 surfaced with their specific blocker for ops follow-up — JOB-260415-039 / JOB-260420-003 / JOB-260417-006 (parts not recorded), JOB-260416-002 / JOB-260416-066 / JOB-260505-056 (checklist not filled), JOB-260428-046 (service notes empty), JOB-260421-017 (approved spare-part not acknowledged). Tech action required on the JobDetail page for those 8.

### Notes

After-photo gate addition to `evaluate_job_completion` is **deferred** — adding it now would retroactively block 3 of the still-stuck jobs that lack after-photos, defeating the recovery itself. Will land in a follow-up PR once the 8 remaining ops-action jobs are cleared. The three-layer doctrine hole stays open ~1 more day; documented as deferred work.

Helper-assignment RLS audit on the legacy `bulkSwipeSignJobs` was a finding from round 1; the new bulk path uses the SECURITY DEFINER atomic RPC and doesn't depend on RLS for writes, so the production path is no longer affected. The legacy single-job sign paths are still RLS-gated and unchanged.

Realtime-echo race in `useJobData.ts` — the atomic RPC means N postgres_changes events per batch instead of 3N writes, so the latent flicker risk on the board is reduced (no longer compounding with the previous 3-write pattern). Tracked separately for a board-level `lastSeenUpdatedAtRef` map.

## [2026-05-07] — Bulk sign-off: technicians could not proceed past photo gate, banner vanished mid-flow, audit trail at risk

Reported by Hafiz Hashim (technician): "after all photo included still cant proceed" on the bulk sign-off banner. Live DB confirmed his two In-Progress jobs each had a `category='after'` photo uploaded, so the gate's premise was correct but its mechanics weren't. A multi-agent adversarial review surfaced three compounding bugs — none user-specific, all production-wide for any technician using bulk sign-off.

### Fixes

**Modal could not see media on board jobs (universal regression).** `BulkSignOffModal`'s `hasAfterPhoto(job)` reads `job.media`, but the JobBoard fetches jobs via `JOB_SELECT.BOARD` which does not include the `media:job_media` join (only `JOB_SELECT.DETAIL` and `DETAIL_FAST` do). Result: every unsigned job — including ones with after-photos correctly uploaded — fell into the disabled "Cannot sign yet — missing after photo" list, pre-selection started empty, and no technician could proceed past step 1. Regression introduced by commit `b381e40` (2026-04-05) which added the after-photo gate to the modal but did not update the BOARD select to load media. Fix: rather than denormalize the BOARD payload (a hot path used by every role on every realtime echo) to feed a niche tech-only gate, added a lazy fetch — new service helper `getAfterPhotoJobIds(jobIds): Promise<Set<string>>` does a single batch query against `job_media` filtered to `category='after'` and returns the qualifying job_id Set. The banner calls it on click, shows a "Loading…" state on the button, and passes the resolved Set to the modal as a prop. The modal replaces its `job.media`-based check with a Set lookup. Architectural rationale: the JOB_SELECT profiles exist precisely to avoid taxing every read path for niche features, and doing so would set the wrong precedent for the next per-feature relation that needs flagging.

**Banner gate hid the recovery path.** `SiteSignOffBanner` only counted `!job.technician_signature` toward `unsignedCount` and required `>= 2` to render. Once a technician worked around the photo-gate bug above by signing individually on each JobDetail page, the banner disappeared even though the customer signature was still missing on the same jobs — there was no UI path back to the bulk customer-signature flow, which was the entire point of the feature. Hafiz had reached exactly this stuck state. Fix: count any job missing **either** signature, so the banner stays visible until both are written.

**`bulkSwipeSignJobs` could overwrite an existing tech signature audit trail.** The service did an unconditional `.update({ technician_signature })` with no idempotency guard. If a technician who had already signed individually re-entered the modal and submitted, their original `signed_at` timestamp would be clobbered with the bulk-time timestamp — destroying the audit trail of when they actually signed. Adversarial review surfaced this before the banner-gate fix shipped, because relaxing the gate (correct fix) would route more partially-signed jobs through the modal and trigger the overwrite (latent bug). Fix: appended `.is('technician_signature', null)` to the per-job UPDATE for `type='technician'` only (customer sigs still always write because re-entry typically captures a different name/IC). Switched the per-job select from `.single()` to `.maybeSingle()` so the no-op case (already-signed → guard matches zero rows) returns `null` instead of throwing; the result map filters nulls. Net effect: the service is now idempotent on tech sigs — calling it on a partially-signed batch silently skips the already-signed jobs and only writes the unsigned ones.

### Changed

**Modal opens directly on the customer step when every job is already tech-signed.** `BulkSignOffModal` now detects the recovery cohort (`techAlreadySignedJobs` — customer-unsigned jobs that already have a tech sig) and, when that's the entire batch, sets `skipTechStep=true`: the technician swipe + selection step is bypassed, the step indicator shows `✓ Technician` instead of `1. Technician`, the back button reads "Cancel" (since there's no step 1 to return to), and the bulk tech-sig service call is omitted on submit. Pre-selection includes both eligible (has after photo, no tech sig) and already-tech-signed jobs so a single customer signature applies to the whole batch in one swipe.

### Notes

Three follow-ups deferred to separate PRs (none user-impacting today):

- **DB trigger doesn't enforce after-photo.** `validate_job_completion_requirements` (latest definition in `20260423_completion_gate_acknowledge_returned.sql`) enforces signatures, hourmeter, checklist, parts confirmation — but **not** `category='after'` photo presence. The gate currently lives only at the UI layer, which violates the project's three-layer doctrine ("DB trigger is authoritative"). A follow-up should add a `PHOTO_REQUIRED_JOB_TYPES` marker in `pages/JobDetail/utils.ts` mirroring the existing `HOURMETER_EXEMPT_JOB_TYPES` pattern, plus a matching IF block in the trigger so any non-UI write path (future API client, scripts, bulk admin actions) cannot bypass the rule.
- **`bulkSwipeSignJobs` has no helper-assignment permission check.** Helpers (`_isHelperAssignment` rows) are eligible for the banner per the 2026-05-05 helper-tech change, but whether their write actually persists depends entirely on the `jobs` UPDATE RLS policy. PostgREST returns no rows on RLS-blocked writes (no error), so a rejected helper write would appear to succeed. Worth an audit of the policy and the service-side guard.
- **Realtime echo + `fetchJobs()` race in `useJobData.ts`.** After bulk-sign completion the banner calls `onComplete` → `fetchJobs()`, which can race the postgres_changes echo of the bulk write. The board's realtime merge applies the join-stripped `jobs`-table row on top of the freshly fetched joined data, briefly stripping `customer` / `forklift` / `customer_site` from the just-signed cards. Distinct from the JobDetail "signal aborted" race (different layer); a board-level `lastSeenUpdatedAtRef` map would close it.

## [2026-05-06] — Client bug sweep: service-tracking divergence, double-deduction, liquid-picker hidden items, METROD account move

Seven separate issues reported by Shin in a single morning thread. Treating them as one release because the diagnostic round (multi-agent adversarial review) surfaced shared root causes — two competing service-completion triggers, a dual-fire deduction path (DB trigger + service layer), a per-row liquid-blind read class — and fixing them piecemeal would have re-broken whatever wasn't touched. Eight SQL migrations applied directly to the live DB with BEGIN…ROLLBACK dry-runs and post-apply invariant checks; minimal TS surface area changed (the DB triggers self-correct most of the user-visible symptoms).

### Fixes

**Service Tracking and Alert show different next-service dates.** Client report: ServiceTracking page shows next service 07/06/2026 (last 09/03/2026 + 90 days) but Alert shows stale 26/03/2026. Root cause: `ServiceTrackingCard.saveEdit` (and three other UI writers) update `last_service_date` but never write `next_service_due`, so the persisted column rots. Plus on the DB side, two completion triggers were in conflict — `trg_service_job_completed` was gated correctly (`job_type IN ('Service','Full Service')`) but never wrote `next_service_due`, while `trigger_update_forklift_service` fired on every status→Completed including Repair with a hardcoded 90-day fallback, leaving 253 forklifts in production with the impossible state `next_service_due < last_service_date`. Fix: dropped the offending Repair-completion trigger; widened `on_service_job_completed` to also stamp `next_service_due` and `next_service_hourmeter` for Service/Full Service/Minor Service/PM Service/PM job types using the `service_intervals` table; added a sibling `BEFORE UPDATE` trigger on forklifts that recomputes the same fields whenever the baseline columns change (covers all 4 UI writers without touching each one). Backfilled 487 forklifts with last_service_date set so the Alert column is now correct everywhere.

**Dashboard treats Electrical forklifts as Diesel (hour-based instead of date-based).** Client report. Root cause: `services/servicePredictionService.ts:isElectricType` matched only `['Electric', 'Battery/Electrical', 'Reach Truck']`. Production has 153 rows typed `'Battery / Electrical'` (with whitespace) that did not match, plus 71 `'Others'` rows that fall on the calendar branch by policy. New `isCalendarServiced(type, fuelType)` helper handles whitespace via `lower().trim()` and accepts an explicit fuel_type. Dashboard fallback now fetches `fuel_type` and routes Electric/Battery/Reach/Others through the calendar branch. Independently: 55 forklifts were found with `type='Battery/Electrical' fuel_type='diesel'` (or LPG / Reach Truck / Others mismatches). Reconciled 51 of them; the 4 'Others'+diesel rows were left for manual review because the `forklifts.fuel_type` CHECK constraint forbids 'other' as a value, and the helper routes them as calendar by `type` regardless.

**Service interval policy update (Diesel 450h, LPG 300h, Electric/Reach/Others 3 months).** Client mandate. The `service_intervals` table already had the correct values for `service_type='Full Service'` (Diesel 450h, LPG 300h, Electric 90 days), but four other sources of truth disagreed: the legacy `set_service_interval_by_type()` trigger encoded 500/350/500, the `get_service_interval(fuel_type)` SQL function returned 500/350/NULL, the TS `|| 500` literal appeared in 6 sites, and `CALENDAR_INTERVAL_DAYS = 90` was duplicated in two files. Fix: new `get_service_interval_for_forklift(p_forklift_id)` SQL helper reads the table; rewrote `set_service_interval_by_type` to keyed-on-fuel_type with new policy; updated `get_service_interval(fuel_type)` for back-compat callers. 1,348 forklifts moved off the old defaults to the new policy values (203 with custom intervals — likely AMC overrides — left untouched).

**JOB-260506-021 REAR HUB SCREW: 3 used, 6 deducted.** Client report. Root cause: dual deduction. `services/vanStockUsageService.useVanStockPart` (called from `useJobPartsHandlers`) decremented `van_stock_items.quantity` and inserted a `van_stock_usage` row, then `addPartToJob` inserted into `job_parts` with `from_van_stock=TRUE`, which fired the legacy `trigger_deduct_van_stock` that decremented again and inserted a second `van_stock_usage` row. Pattern was double-firing on every solid van-stock use since the service-layer deduction was added — production-wide there were 22 dup pairs across 13 items consuming 24 phantom units. Fix: dropped the legacy trigger; new atomic RPC `rpc_use_van_stock_part` does decrement (`FOR UPDATE` row lock + atomic UPDATE-WHERE-RETURNING per the existing `reserve_part_stock` pattern) + `van_stock_usage` insert + `inventory_movements` insert + `job_parts` insert in one transaction. Idempotency via UNIQUE partial index on `van_stock_usage.idempotency_key` — same key returns the same `job_parts` row, no double-decrement on retry. Customer-fleet approval gate (`forklifts.ownership='customer'`) ported from the dropped trigger into the RPC. Cleaned up the 22 existing dup pairs and refunded 24 units (REAR HUB SCREW now correctly shows 3 actual deductions in `inventory_movements`).

**JOB-260506-034 OIL FILTER 1631 deducted but not in Parts Used.** Client report. Same handler had a second bug: `useVanStockPart` and `addPartToJob` were two independent PostgREST calls with no transactional wrapper. If the second threw (RLS, validation, network), the first had already committed and was never reversed — leaving an orphan: stock decremented, no `job_parts` row. Production-wide there were 31 such orphans across 17 jobs since 2026-04-01. Fix: the same atomic RPC eliminates the class — both writes share one transaction. Cleaned up the 31 existing orphans and refunded 37 units; phantom `inventory_movements` rows deleted.

**Liquid items missing from technician's part picker (Gear Oil, Hydraulic Oil etc., 3 vans).** Client report. Root cause: `pages/JobDetail/components/PartsSection.tsx:415` filtered with `i.quantity > 0`, but the existing `route_liquid_to_bulk_quantity` trigger forces `van_stock_items.quantity` to 0 for every liquid SKU (real liquid stock lives in `container_quantity * parts.container_size + bulk_quantity`). Fix: added a maintained column `van_stock_items.effective_quantity` with a BEFORE INSERT/UPDATE trigger that computes the dual-unit formula for liquids or `quantity` for solids, plus a sibling AFTER UPDATE trigger on `parts` that re-stamps van_stock_items rows when `is_liquid` or `container_size` changes (so the column doesn't rot when liquid attributes change). New `utils/vanStock.ts:getVanStockAvailableQty` helper consumed by the picker filter and the consumption availability check. Backfill restored visibility on 50 liquid rows that had previously been hidden.

**Move 4 forklifts from METROD M50 to M55.** Client report: METROD has two accounts (M50 / M55) for different locations and 4 units (`7FBR13-12312`, `7FD45-38065`, `8FD30-73970`, `8FD15-60424`) were placed under M50 by mistake. Both customer rows share `name='METROD (MALAYSIA) SDN BHD'` and are distinguished only by `account_number` (`3000/M50` vs `3000/M55`). Updated `forklifts.current_customer_id` for the 4 units; `customer_id` was NULL on all 4 and stayed NULL (these are `ownership_type='fleet'` rentals). Pre/post sanity counts confirmed.

### Changed

**Admin dashboard low-stock count uses `parts.effective_stock`.** The generated column from `20260501_search_perf_trigram_and_low_stock.sql` already had the right semantics (liquid: container_qty + bulk_qty/container_size; solid: stock_quantity); the dashboard hook just wasn't reading it. Sealed liquid drums no longer show as out-of-stock to admins.

### Notes

Five of six atomicity holes in van-stock-related handlers deferred to a follow-up: `rejectVanStockUsage`, `addPartToJob` non-liquid main-store, `addPartToJob` liquid `sellContainersExternal`, `liquidInventoryService.useVanBulk`, and `approveSparePartRequest` (which already has a compensating `rollback_part_stock`). The `v_forklift_service_predictions` view was not rewritten — PR 1's backfill makes the columns it reads correct, so downstream consumers (the daily cron, external-fleet tab) self-correct.
