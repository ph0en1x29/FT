...
const ForkliftProfile: React.FC<ForkliftProfileProps> = ({ currentUser }) => {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      {/* Service History Card - matches Customer Profile "Rented Forklifts" style */}
      <div className="bg-white rounded-xl shadow-sm p-6">
        <h3 className="font-bold text-slate-800 flex items-center gap-2 mb-4">
          <Wrench className="w-5 h-5 text-blue-600" /> Service History ({serviceHistory.length})
        </h3>
          ...
          <div className="space-y-3 max-h-64 overflow-y-auto">
            {rentals.map(rental => (
              <div 
                ...
                className={`p-4 rounded-lg border cursor-pointer transition hover:shadow-sm ${
              ...
              >
                <div className="flex justify-between items-start">
                  <div>
                    <div className="flex items-center gap-2">
                      <Building2 className={`w-4 h-4 ${rental.status === 'active' ? 'text-green-600' : 'text-slate-400'}`} />
                      <span className="font-medium text-slate-800">{rental.customer?.name}</span>
                      {rental.status === 'active' && (
