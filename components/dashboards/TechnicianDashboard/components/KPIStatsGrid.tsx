...
export const KPIStatsGrid: React.FC<KPIStatsGridProps> = ({
  ...
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-4">
      {/* Today's Jobs */}
      <div
        className="card-premium p-3 sm:p-5 border-l-4 border-l-[var(--accent)] cursor-pointer hover:shadow-lg transition-all min-w-0 overflow-hidden"
      ...
      >
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-2xl sm:text-3xl font-bold mt-2 text-[var(--text)]">{todayJobs.length}</p>
            <p className="text-xs mt-1 text-[var(--text-subtle)]">Scheduled for today</p>
          </div>
          <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-xl bg-[var(--accent-subtle)] flex items-center justify-center">
      ...
      {/* In Progress */}
      <div
        className={`card-premium p-3 sm:p-5 border-l-4 cursor-pointer hover:shadow-lg transition-all min-w-0 overflow-hidden ${
      ...
      {/* Completed This Week */}
      <div className="card-premium p-3 sm:p-5 border-l-4 border-l-green-500 min-w-0 overflow-hidden">
        <div className="flex items-start justify-between">
          <div>
            <p className="text-xs font-medium uppercase tracking-wide text-[var(--text-muted)]">
              Completed
            </p>
            <p className="text-2xl sm:text-3xl font-bold mt-2 text-green-600">{completedThisWeek.length}</p>
      ...
      >
        <div className="flex items-start justify-between">
          ...
          >
            <Package className={`w-4 h-4 sm:w-5 sm:h-5 ${vanStockLow > 0 ? 'text-orange-600' : 'text-green-600'}`} />
          </div>
        </div>
      </div>
    </div>
