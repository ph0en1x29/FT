...
-- Fix for hr_alerts column name error
-- Date: 2026-01-03
-- ============================================
...
-- ============================================
-- TABLE: hr_alerts (FIXED - uses recipient_ids array)
-- ============================================
DROP POLICY IF EXISTS "alerts_admin_all" ON hr_alerts;
DROP POLICY IF EXISTS "alerts_select_recipient" ON hr_alerts;

-- Consolidated: Users see alerts where they are in recipient_ids OR Admin sees all
CREATE POLICY "hr_alerts_select_policy" ON hr_alerts
  FOR SELECT TO authenticated
  USING (
    (SELECT user_id FROM users WHERE auth_id = (select auth.uid())) = ANY(recipient_ids)
    OR (select get_my_role()) IN ('Admin', 'Supervisor')
  );
...
-- 5. FIXED: hr_alerts uses recipient_ids array with ANY() operator
-- ============================================
